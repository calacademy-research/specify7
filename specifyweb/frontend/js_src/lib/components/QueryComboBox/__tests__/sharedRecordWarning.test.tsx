...
describe('Shared record edit warning', () => {
  test('localization strings exist for the warning dialog', () => {
    ...
    expect(typeof formsText.rememberChoiceForSession()).toBe('string');
    expect(formsText.rememberChoiceForSession().length).toBeGreaterThan(0);

    expect(typeof formsText.linksForInspectionOnly()).toBe('string');
    expect(formsText.linksForInspectionOnly().length).toBeGreaterThan(0);
  });

  test('overflow message includes the remaining record count', () => {
    const overflow = formsText.andNMore({ count: '7' });
    expect(typeof overflow).toBe('string');
    expect(overflow).toContain('7');
  });
