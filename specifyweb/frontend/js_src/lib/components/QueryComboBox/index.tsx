        ...
        (canResolveToCollectionObject
          ...
          : /* Fall back to querying the direct parent table */
            ajax<{
              ...
              records: data.objects.slice(0, 10).map((obj) => ({
                parentId: obj.id,
                parentLabel: `${resource!.specifyTable.label} #${obj.id}`,
                parentTableName: resource!.specifyTable.name,
                sharedId: relatedId,
                sharedTableName: field.relatedTable.name,
              })),
            }))
  ...
  const doCloneAndEdit = (): void => {
    ...
    loading(
      relatedResource.clone(true).then((clonedResource) => {
        /*
         * The parent is updated in the AddResourceState onSaved handler,
         * so a cancelled dialog leaves the parent record unchanged.
         */
        setState({
